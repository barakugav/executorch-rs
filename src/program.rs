...
impl<'a> Program<'a> {
    ...
    pub fn load(
        ...
        let program = try_new(|program| unsafe {
            et_rs_c::executorch_Program_load(data_loader, verification, program)
        })?;
    ...
    pub fn get_method_name(&self, method_index: usize) -> Result<&str> {
        let method_name = try_new(|method_name| unsafe {
            et_rs_c::executorch_Program_get_method_name(&self.0, method_index, method_name)
        })?;
    ...
    pub fn load_method<'b>(
        ...
        let method = try_new(|method| unsafe {
            et_rs_c::executorch_Program_load_method(
                &self.0,
                method_name.as_ptr(),
                memory_manager,
                event_tracer,
    ...
    pub fn method_meta(&self, method_name: &CStr) -> Result<MethodMeta> {
        let meta = try_new(|meta| unsafe {
            et_rs_c::executorch_Program_method_meta(&self.0, method_name.as_ptr(), meta)
        })?;
...
impl Drop for Program<'_> {
    fn drop(&mut self) {
        unsafe { et_rs_c::executorch_Program_destructor(&mut self.0) };
    }
}
...
pub struct MethodMeta<'a>(et_rs_c::MethodMeta, PhantomData<&'a ()>);
impl MethodMeta<'_> {
    pub(crate) unsafe fn new(meta: et_rs_c::MethodMeta) -> Self {
        Self(meta, PhantomData)
    }

    /// Get the name of this method.
    pub fn name(&self) -> &str {
        let name = unsafe { et_rs_c::executorch_MethodMeta_name(&self.0) };
        unsafe { CStr::from_ptr(name).to_str().unwrap() }
    }

    /// Get the number of inputs to this method.
    pub fn num_inputs(&self) -> usize {
        unsafe { et_rs_c::executorch_MethodMeta_num_inputs(&self.0) }
    }
    ...
    pub fn input_tag(&self, idx: usize) -> Result<Tag> {
        try_new(|tag| unsafe { et_rs_c::executorch_MethodMeta_input_tag(&self.0, idx, tag) })
    }
    ...
    pub fn input_tensor_meta(&self, idx: usize) -> Result<TensorInfo> {
        let info = try_new(|info| unsafe {
            et_rs_c::executorch_MethodMeta_input_tensor_meta(&self.0, idx, info)
        })?;
    ...
    pub fn num_outputs(&self) -> usize {
        unsafe { et_rs_c::executorch_MethodMeta_num_outputs(&self.0) }
    }
    ...
    pub fn output_tag(&self, idx: usize) -> Result<Tag> {
        try_new(|tag| unsafe { et_rs_c::executorch_MethodMeta_output_tag(&self.0, idx, tag) })
    }
    ...
    pub fn output_tensor_meta(&self, idx: usize) -> Result<TensorInfo> {
        let info = try_new(|info| unsafe {
            et_rs_c::executorch_MethodMeta_output_tensor_meta(&self.0, idx, info)
        })?;
        Ok(unsafe { TensorInfo::new(info) })
    }
    ...
    pub fn num_memory_planned_buffers(&self) -> usize {
        unsafe { et_rs_c::executorch_MethodMeta_num_memory_planned_buffers(&self.0) }
    }
    ...
    pub fn memory_planned_buffer_size(&self, idx: usize) -> Result<usize> {
        let size = try_new(|size| unsafe {
            et_rs_c::executorch_MethodMeta_memory_planned_buffer_size(&self.0, idx, size)
        })?;
        Ok(size as usize)
    }
}
...
pub struct TensorInfo<'a>(et_rs_c::TensorInfo, PhantomData<&'a ()>);
impl<'a> TensorInfo<'a> {
    pub(crate) unsafe fn new(info: et_rs_c::TensorInfo) -> Self {
        Self(info, PhantomData)
    }

    /// Returns the sizes of the tensor.
    pub fn sizes(&self) -> &'a [i32] {
        let span = unsafe { et_rs_c::executorch_TensorInfo_sizes(&self.0) };
        unsafe { ArrayRef::from_inner(span) }.as_slice()
    }

    /// Returns the dim order of the tensor.
    pub fn dim_order(&self) -> &'a [u8] {
        let span = unsafe { et_rs_c::executorch_TensorInfo_dim_order(&self.0) };
        unsafe { ArrayRef::from_inner(span) }.as_slice()
    }

    /// Returns the scalar type of the input/output.
    pub fn scalar_type(&self) -> Option<ScalarType> {
        let scalar_type = unsafe { et_rs_c::executorch_TensorInfo_scalar_type(&self.0) };
        ScalarType::from_c_scalar_type(scalar_type)
    }

    /// Returns the size of the tensor in bytes.
    pub fn nbytes(&self) -> usize {
        unsafe { et_rs_c::executorch_TensorInfo_nbytes(&self.0) }
    }
}
...
pub struct Method<'a>(et_rs_c::Method, PhantomData<&'a ()>);
impl Method<'_> {
    ...
    pub fn inputs_size(&self) -> usize {
        unsafe { et_rs_c::executorch_Method_inputs_size(&self.0) }
    }
}
impl Drop for Method<'_> {
    fn drop(&mut self) {
        unsafe { et_rs_c::executorch_Method_destructor(&mut self.0) };
    }
}

/// An method execution builder used to set inputs and execute the method.
pub struct Execution<'a> {
    method: &'a mut et_rs_c::Method,
    set_inputs: u64,
}
impl<'a> Execution<'a> {
    fn new(method: &'a mut et_rs_c::Method) -> Self {
        assert!(
            unsafe { et_rs_c::executorch_Method_inputs_size(method) } <= u64::BITS as usize,
    ...
    pub fn execute(self) -> Result<Outputs<'a>> {
        assert_eq!(
            self.set_inputs,
            (1 << unsafe { et_rs_c::executorch_Method_inputs_size(self.method) }) - 1,
            "some inputs were not set"
        );
        unsafe { et_rs_c::executorch_Method_execute(self.method) }.rs()?;
...
impl<'a> Outputs<'a> {
    ...
    pub fn len(&self) -> usize {
        unsafe { et_rs_c::executorch_Method_outputs_size(self.method) }
    }
