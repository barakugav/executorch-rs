...
/// ```rust,ignore
/// // Struct generated by bindgen with unsafe interface
/// #[repr(C)]
/// #[repr(align(8))]
...
/// usual.
///
/// The `executorch` crate encapsulates Cpp objects which are trivially movable in Rust objects using the a simple
/// `struct RustString(StdString)`, similar to the first example, but never does so for non-trivially movable objects.
...
///
/// Non-trivially movable objects such as `Tensor` and `EValue` expose a straight forward `new` function that allocate
/// the object on the heap which should be used in most cases. The `new` function is available when the `alloc` feature
/// is enabled. When allocations are not available, the `Storage` struct should be created and pinned, which expose an
/// identical `new` function that allocates the object in place:
/// ```rust,ignore
